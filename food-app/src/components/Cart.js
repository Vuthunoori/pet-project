...
const CartModal = ({ visible, onClose }) => {
  ...
  const handleDecreaseQuantity = (index) => {
    setCartItems(prevCartItems => {
      ...
      ).filter((item, i) => !(i === index && prevCartItems[index].quantity === 1));
      return updatedCartItems;
    });
  };
  ...
  const handlePayment = () => {
    navigate("/pay");// navigation logic to payment page if needed
  };
          ...
          {cartItems.map((item, index) => (
            <li key={index} style={{ marginBottom: '20px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center' }}>
                ...
                <Button style={{ marginLeft: '10px' }} type="primary" onClick={() => handleDecreaseQuantity(index)}>-</Button>
                <p style={{ margin: '0 10px' }}>{item.quantity}</p>
                <Button type="primary" onClick={() => handleIncreaseQuantity(index)}>+</Button>
                <Button type="primary" onClick={() => handleRemoveItem(index)} style={{ marginLeft: '20px' }}>
                  Remove
                </Button>
              </div>
            </li>
